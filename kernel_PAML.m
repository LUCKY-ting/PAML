    ...
    for o = 1:epoch
        ...
        for i=1:n
            ...
            for iter = 1:maxIterNum
                ...
                if isnan(f_t_1)
                    alpha = 0;
                    beta = max(0, f_t_2 / (2 * snx));
                elseif isnan(f_t_2)
                    alpha = max(0, f_t_1 / (2 * snx));
                    beta = 0;
                elseif f_t_1 <= 0 && f_t_2 <= 0
                    alpha = 0;
                    beta = 0;
                elseif f_t_1 > 0 && f_t_2 <= -1/2 * f_t_1
                    alpha = f_t_1 / (2 * snx);
                    beta = 0;
                elseif f_t_2 > 0 && f_t_1 <= -1/2 * f_t_2
                    alpha = 0;
                    beta = f_t_2 / (2 * snx);
                else
                ...
                %                     fprintf('r_t = %d, s_t = %d, alpha = %f, beta = %f \n ', r_t, s_t,alpha,beta)
            end
        end
    end
...
fid = fopen('kernel_PAML(running_time).txt','a');
fprintf(fid,'name = scene, kernel_PAML, runTimes = %d, epoch = %d, scale = %g \n', times, epoch, scale);
