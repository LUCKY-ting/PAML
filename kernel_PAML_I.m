    ...
    for o = 1:epoch
        ...
        for i=1:n
            ...
            for iter = 1:maxIterNum
                ...
                if isnan(f_t_1)
                    alpha = 0;
                    beta = min(max(0, f_t_2 / (2 * snx)), C);
                elseif isnan(f_t_2)
                    alpha = min(max(0, f_t_1 / (2 * snx)), C);
                    beta = 0;
                elseif f_t_1 <= 0 && f_t_2 <= 0
                    alpha = 0;
                    beta = 0;
                elseif f_t_1 <= -0.5 * f_t_2 && f_t_2>0 && f_t_2 <= 2*C*snx
                    alpha = 0;
                    beta = f_t_2 / (2 * snx);
                elseif f_t_1 <= -C*snx && f_t_2 > 2*C*snx
                    alpha = 0;
                    beta = C;
                elseif f_t_1 > -C*snx && f_t_1 <= C*snx && f_t_2 >= -0.5*f_t_1 + 3*C*snx/2
                    alpha = 0.5 * (C + f_t_1/snx);
                    beta = C;
                elseif f_t_1 > C*snx && f_t_2 > C*snx
                    alpha = C;
                    beta = C;
                elseif f_t_2 > -C*snx && f_t_2 <= C*snx && f_t_1 >= -0.5*f_t_2 + 3*C*snx/2
                    alpha = C;
                    beta = 0.5 * (C + f_t_2/snx);
                elseif f_t_2 <= -C*snx && f_t_1 > 2*C*snx
                    alpha = C;
                    beta = 0;
                elseif f_t_2 <= -0.5 * f_t_1 && f_t_1 > 0 && f_t_1 <= 2*C*snx
                    alpha = f_t_1 / (2 * snx);
                    beta = 0;
                else
...
fid = fopen('kernel_PAML_I_(running_time).txt','a');
fprintf(fid,'name = scene, kernel_PAML_I, runTimes = %d, epoch = %d, scale = %g, C = %g\n', times, epoch, scale, C);
